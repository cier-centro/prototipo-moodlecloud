...
function createQuestion() {
  ...
  for (i = 0; i < activityContent.options[currentQuestion].labels.length; i++) {
    $(CheckListOpt).append(
      '<div class="custom-control custom-checkbox" id="check_' + (i + 1) + '" data-toggle="tooltip" title="Opción">'+
        '<input type = "checkbox" id = "checkBtn_' + (i + 1) + '" name = "question_' + currentQuestion + '" value = "' + (i + 1) + '" class= "custom-control-input" >' +
        '<label class="custom-control-label" for="checkBtn_' + (i + 1) + '"></label>' +
      '</div > '
    );
  }
